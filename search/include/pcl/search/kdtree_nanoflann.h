 ...
 */
template <typename T = float> // typename CloudT,
...
template <typename PointT,
          ...
          typename Tree = nanoflann::KDTreeSingleIndexAdaptor<
              ...
              pcl::search::internal::PointCloudAdaptor<float>,
              Dim,
              pcl::index_t>> // TODO static vs dynamic?
...
public:
   ...
   */
  KdTreeNanoflann(bool sorted, std::size_t leaf_max_size)
   ...
   *
   */
  KdTreeNanoflann(bool sorted = false) : KdTreeNanoflann(sorted, 15) {}
                 ...
                 std::vector<float>& k_sqr_distances) const override
  {
    ...
    if (static_cast<std::size_t>(k) >
        adaptor_
            ->kdtree_get_point_count()) // TODO maybe additionally store point cloud, to
                                        // avoid dereferencing adaptor_ over and over?
    ...
    const auto search_result = nanoflann_tree_->knnSearch(
    ...
    if (max_nn == 0) {
      ...
      if (sorted_results_) {
...
#endif
        const auto search_result = nanoflann_tree_->radiusSearch(
            (query_point ? query_point : reinterpret_cast<const float*>(&point)),
        ...
        PCLRadiusResultSet<float, pcl::index_t> resultSet(
            ...
            k_indices,
            k_sqr_distances);
        const auto search_result = nanoflann_tree_->radiusSearchCustomCallback(
            (query_point ? query_point : reinterpret_cast<const float*>(&point)),
            resultSet,
#if NANOFLANN_VERSION < 0x150
            {32, eps_, sorted_results_} // first parameter is ignored in older versions,
      ...
      if (use_rknn_) {
        ...
        // rknnSearch was added in nanoflann 1.5.1
...
#else
        const auto search_result = nanoflann_tree_->rknnSearch(
            (query_point ? query_point : reinterpret_cast<const float*>(&point)),
            max_nn,
            k_indices.data(),
            k_sqr_distances.data(),
...
private:
  ...
  setUpTree()
  {
    ...
    if (Dim != -1 && Dim != static_cast<int>(dim)) {
      PCL_ERROR("[setInputCloud] The given point representation %i, %i\n", Dim, dim);
      return false;
    }
    PCL_DEBUG("[setInputCloud] identity_mapping_=%i, "
    ...
    nanoflann_tree_.reset(new Tree(dim, *adaptor_, {leaf_max_size_}));
    return true;
  }
